...
export default function NewsItem2({search}: { search: string }) {
        ...
        async function getAllNews() {
            try {
                const url = search.trim()
                    ? `http://localhost:8888/public/news?search=${search}`
                    : `http://localhost:8888/public/news`;
                ...
                console.error("Yangiliklarni yuklashda xatolik yuz berdi:", error);
                setNews([]);
            }
        }
